...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.error("[sync/push] failed", {
      userId,
      collections: collections ? Object.fromEntries(Object.entries(collections).map(([key, value]) => [key, Array.isArray(value) ? value.length : 0])) : {},
      message,
      stack: error instanceof Error ? error.stack : undefined,
    });
    return sendJson(res, 500, { error: message, code: "SYNC_PUSH_FAILED" });
  }
};
