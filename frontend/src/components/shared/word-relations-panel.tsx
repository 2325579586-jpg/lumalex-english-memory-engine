      ...
      {detailOpen ? (
        <div className="mt-3 rounded-2xl border border-border/70 bg-panel/70 p-3 text-xs leading-5 text-muted-foreground">
          ...
          <p className="mt-2">
            ...
            {word.meaning || "暂未找到更多释义。"}
          </p>
          {word.commonMeanings.length ? <p className="mt-2">常见义：{word.commonMeanings.join("；")}</p> : null}
          {word.examMeanings.length ? <p className="mt-1">常考义：{word.examMeanings.join("；")}</p> : null}
          {word.example ? (
            <div className="mt-2 border-t border-border/60 pt-2">
              <p>{word.example}</p>
              {word.exampleTranslation ? <p className="mt-1">{word.exampleTranslation}</p> : null}
            </div>
          ) : (
...
export function WordRelationsPanel({
  ...
  const lookupEntriesRef = useRef(lookupEntries);
  const lookupLoadingRef = useRef(lookupLoading);
  ...
  useEffect(() => {
    lookupEntriesRef.current = lookupEntries;
  }, [lookupEntries]);
  ...
  useEffect(() => {
    ...
    const missingTerms = visibleTerms.filter((term) => {
      ...
      return !lookupEntriesRef.current[key] && !lookupLoadingRef.current.has(key);
    });
  ...
  const viewDetail = (term: string) => {
    setDetailTerm((current) => (normalize(current) === normalize(term) ? "" : term));
    setMessage("");
  };
  ...
  const openRelatedWord = (word: RelatedWord) => {
    ...
    setDetailTerm("");
    const key = normalize(word.term);
    if (lookupEntriesRef.current[key] || lookupLoadingRef.current.has(key)) return;
    ...
    <div className="overflow-hidden rounded-[22px] border border-border/70 bg-panel/60">
          ...
          <div>
            {example.trim() ? (
              <div className="rounded-2xl bg-white/5 p-3">
                ...
                <ClickableSentence sentence={example} translation={exampleTranslation} deckId={deckId} preferredAccent={preferredAccent} className="mt-2" />
      ...
      {selectedWordForCard ? (
        <RelatedWordCard
          ...
          detailOpen={normalize(detailTerm) === normalize(selectedWordForCard.term)}
